...
class CreateNewReservationActivity : AppCompatActivity() {
    companion object{
        //Extra
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_create_new_reservation)

        requestStoragePermission()
        val space_id_selected=intent.getIntExtra(CreateNewUsingSpaceActivity.SPACE_SELECTED,-1)
        val parking_lot_selected=intent.getStringExtra(CreateNewUsingSpaceActivity.PARKING_LOT_SELECTED)


        findViewById<ImageView>(R.id.admin_reservation_photo_upload).setOnClickListener {
        ...
        findViewById<Button>(R.id.btn_admin_reservation_add_finish).setOnClickListener {
            ...
            if(identity>=0){
                ...
                else usingSpace.parking_id= findViewById<EditText>(R.id.admin_reservation_cardID).text.toString()
                ...
                CoroutineScope(Dispatchers.Main).launch {
                    withContext(Dispatchers.IO){
                        val spaceAvail = mysqlHelper.getParkLotInfoByName(usingSpace.parking_lot).space_available
            ...
            fromAlbum -> {
                if (resultCode == Activity.RESULT_OK && data != null) {
                    data.data?.let { uri ->
                        ...
                        findViewById<ImageView>(R.id.admin_reservation_photo_upload).setImageBitmap(bitmap)
                        ...
                        try {
                            CoroutineScope(Dispatchers.Main).launch {
                                findViewById<ProgressBar>(R.id.admin_reservation_photo_upload_progressbar).visibility =
                                    View.VISIBLE
                                withContext(Dispatchers.IO){
